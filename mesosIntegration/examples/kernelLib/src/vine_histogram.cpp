...
int vineFunCallerInterface::vine_histogram(int hist_version)//(int argc, char **argv) 
{
  uchar *h_Data;
...
// set logfile name and start logs
  ...
  */
  //Function Argument to specify which histogram version to run
  // 0 is for 64 and 1 is for 256
  if(hist_version==0){
	histogramVersion=hist_version;
	printf(" Histogram 64 bin version.  \n");
  }
  else if(hist_version==1){
  	histogramVersion=hist_version;
        printf(" Histogram 256 bin version.  \n");
  }else{
        printf("Invalid Histogram Version value.\n Will execute default histogram 256 bin version");
	histogramVersion=1;
  }
  /*
